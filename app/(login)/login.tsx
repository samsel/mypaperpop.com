...
export function Login({ mode = 'signin' }: { mode?: 'signin' | 'signup' }) {
  ...
  return (
    <div className="b-paper relative flex min-h-[100dvh] items-center justify-center overflow-hidden bg-[var(--paper)]">
      {/* Immersive Background */}
      <div className="absolute inset-0 z-0">
        {/* Gradient Overlay for Fade Effect - Top and Bottom */}
        <div className="absolute inset-0 z-10 bg-gradient-to-b from-[var(--paper)] via-transparent to-[var(--paper)] opacity-90" />

        {/* White Overlay for dimming */}
        <div className="absolute inset-0 z-20 bg-[var(--paper)]/70 backdrop-blur-[2px]" />

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 h-full p-4 overflow-hidden opacity-50">
          {/* Create multiple columns of scrolling content for a "wall" effect */}
          {Array.from({ length: 4 }).map((_, columnIndex) => (
            <div
              key={columnIndex}
              className={`flex flex-col gap-6 animate-vertical-scroll ${
                // Stagger animations slightly for natural feel
                columnIndex % 2 === 0 ? '' : 'animation-delay-2000'
                }`}
              style={{
                // Add random offsets or delays if possible, purely CSS for now
                animationDuration: columnIndex % 2 === 0 ? '40s' : '55s'
              }}
            >
              ...
              {[...galleryItems, ...galleryItems, ...galleryItems].map((item, idx) => (
                <div
                  key={`${columnIndex}-${idx}`}
                  className="overflow-hidden rounded-md border-[1.5px] border-[var(--ink)] bg-white"
                >
                  <img
                    src={item.src}
                    alt=""
      ...
      <div className="relative z-30 w-full max-w-md px-4">
        <div className="paper-sheet bg-white px-8 py-10">
